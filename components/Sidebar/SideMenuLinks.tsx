...
// constants
import sideMenuLinkOptions from 'constants/sideMenuLinkOptions';

// types
import { MenuList, LinkType } from 'interfaces/SideMenuLink';
          ...
          return (
            <div key={menuLink.categoryName}>
              <div className="bb bt b--black-10 flex items-center ph3 h3">
                <img src={`/static/svgs/${menuLink.iconName}.svg`} className="w1 h1 fill-black-30" />
        ...
        menuLink.links.map((link) => {
          return (
            <MenuLink
              key={link.route}
              link={link}
              selected={selectedRoute.startsWith(link.route)}
              isMobile={isMobile}
            />
          );
        })
